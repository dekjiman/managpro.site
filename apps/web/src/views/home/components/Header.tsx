...
const Header = ({ isLoggedIn }: { isLoggedIn: boolean }) => {
  ...
  const mobileMenuItems = [
    ...
    { label: t`Fitur`, href: "/#features", group: "Produk" },
    { label: t`Harga`, href: "/pricing", group: "Produk" },
    { label: t`FAQ`, href: "/#faq", group: "Sumber Daya" },
    { label: t`Kontak`, href: "mailto:support@kan.bn", group: "Sumber Daya" },
  ];
          ...
          <div className="mx-auto flex w-full max-w-[1100px] items-center justify-between lg:px-4">
            ...
            <div className="flex items-center justify-end gap-4 lg:w-[200px]">
              <div className="justify-end gap-2">
                {isLoggedIn ? (
                  <Button href="/boards">{t`Buka Aplikasi`}</Button>
                ) : (
                  <div className="flex items-center justify-end gap-2">
                    <Button href="/login" variant="ghost">
                      {t`Masuk`}
                    </Button>
                    <Button href="/signup">{t`Daftar`}</Button>
                  </div>
                )}
              </div>
