...
export function Auth({
  ...
  const verifyTurnstile = useCallback(async (): Promise<boolean> => {
    ...
    if (!turnstileToken) {
      setLoginError(t`Silakan selesaikan verifikasi CAPTCHA.`);
      return false;
    }
    try {
      ...
      if (!data.success) {
        setLoginError(t`Verifikasi CAPTCHA gagal. Silakan coba lagi.`);
  ...
  const handleLoginWithEmail = async (
    ...
    if (password) {
        ...
        await authClient.signUp.email(
          ...
          },
          {
            onSuccess: () =>
              showPopup({
                header: t`Berhasil`,
                message: t`Anda berhasil mendaftar.`,
                icon: "success",
              }),
      ...
      if (isCloudEnv || (isEmailSendingEnabled && !isSignUp)) {
        ...
        setLoginError(
          isSignUp
            ? t`Kata sandi diperlukan untuk mendaftar.`
            : t`Kata sandi diperlukan untuk masuk.`,
        );
      }
    }
  ...
  const handleLoginWithProvider = async (provider: AuthProvider) => {
    ...
    if (error) {
      setLoginError(
        t`Gagal masuk dengan ${provider.at(0)?.toUpperCase() + provider.slice(1)}. Silakan coba lagi.`,
      );
    }
  };
        ...
        socialProviders?.length === 0 && (
          <div className="flex w-full items-center gap-4">
            ...
            <span className="text-center text-sm text-light-900 dark:text-dark-900">
              {t`Tidak ada metode autentikasi yang tersedia`}
            </span>
            <div className="h-[1px] w-1/3 bg-light-600 dark:bg-dark-600" />
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="space-y-2">
            ...
            <div>
              <Input
                ...
                placeholder={t`Masukkan alamat email Anda`}
              />
              {errors.email && (
                <p className="mt-2 text-xs text-red-400">
                  {t`Masukkan alamat email yang valid`}
                </p>
              )}
            </div>
            ...
            {isCredentialsEnabled && (
              <div>
                <Input
                  ...
                  placeholder={t`Masukkan kata sandi Anda`}
                />
                {errors.password && (
                  <p className="mt-2 text-xs text-red-400">
                    {errors.password.message ??
                      t`Masukkan kata sandi yang valid`}
                  </p>
                )}
              </div>
            )}
          ...
          <div className="mt-[1.5rem] flex items-center gap-4">
            <Button
              ...
              variant="secondary"
            >
              {isSignUp ? t`Daftar dengan ` : t`Masuk dengan `}
