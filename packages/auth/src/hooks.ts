    ...
    user: {
      create: {
        async before(user: BetterAuthUser, _context: unknown) {
          const signUpDisabled = process.env.NEXT_PUBLIC_DISABLE_SIGN_UP;
          const allowedDomains = process.env.BETTER_AUTH_ALLOWED_DOMAINS;

          log.debug({ signUpDisabled, allowedDomains, email: user.email }, "user.create.before hook");

          if (signUpDisabled?.toLowerCase() === "true") {
            ...
            if (!pendingInvitation) {
              log.warn({ email: user.email }, "Sign-up disabled and no pending invitation found");
              return Promise.resolve(false);
            }
          }
          if (allowedDomains) {
            const allowed = allowedDomains.split(",")
              .map((d) => d.trim().toLowerCase())
              .filter(Boolean);
            if (allowed.length > 0) {
              const domain = user.email.split("@")[1]?.toLowerCase();
              if (!domain || !allowed.includes(domain)) {
                log.warn({ email: user.email, domain, allowed }, "Email domain not allowed");
                return Promise.resolve(false);
              }
            }
          }
